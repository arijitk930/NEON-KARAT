...
import { Modal } from "../pages/RegisterPage";
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between items-center w-full max-w-[77rem] mx-auto px-4 sm:px-10 py-10 space-y-10 sm:space-y-0 sm:space-x-10">
      ...
      <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
        <button onClick={()=>{showModalMessage()}} className="px-6 py-2 border border-gray-900 text-gray-900 font-semibold hover:bg-black hover:text-white transition duration-200">
          APPLY FOR CARD
        </button>
        <button onClick={()=>{showModalMessage()}} className="px-6 py-2 border border-gray-900 text-gray-900 font-semibold hover:bg-black hover:text-white transition duration-200">
      ...
      {showModal && (
        <Modal message="Comming soon..." onClose={closeModal} />
      )}
    </div>
  );
};
